        ...
        try {
            ...
            try{
                if(getUserByName(user.getUsername())!=null) {
                    JOptionPane.showMessageDialog(roomFrm, "Tài khoản này đang đăng nhập!");
                    return;
                }
            }catch(Exception ex){
...
//                loginFrm.setVisible(false);
    ...
    public void requestJoinRoom(int roomID){
        try{
            ...
            if(isJoined(roomID,currentUser)) {
                JOptionPane.showMessageDialog(roomFrm, "Bạn đã ở trong phòng này!");
                return;
            }
    ...
    public void deleteRoom(int roomID){
        try {
            ...
            if(!roomBoss.getUsername().equals(currentUser.getUsername())){
                JOptionPane.showMessageDialog(roomFrm, "Bạn không có quyền xóa phòng");
                return;
            }
    ...
    public boolean addUserToRoom(int userID, int roomID){
        ...
        if(aRoom.getStatus().contains("full")){
            JOptionPane.showMessageDialog(roomFrm, "Phòng đã đủ người!");
            return false;
        }
        ArrayList<User> aList = aRoom.getUserList();
...
//                        System.out.println("trigger start");
                    ...
                    case "SOLO":
                        ...
                        int input = JOptionPane.showConfirmDialog(roomFrm, "Bạn có muốn chơi với "+ rivalName, "Xác nhận thách đấu", JOptionPane.OK_CANCEL_OPTION);
                    ...
                    case "JOIN":
                        ...
                        if(loggedUsers.get(userID).getUsername().equals(currentUser.getUsername())){
                            ...
                            input = JOptionPane.showConfirmDialog(roomFrm, "Bạn có muốn người chơi "+userName+" nhập phòng "+roomName,"Xác nhận nhập phòng",JOptionPane.OK_CANCEL_OPTION);
...
//                        oos.flush();
                    ...
                    case "REPJOIN":
                        ...
                        if(msg.contains("OK")){
                            JOptionPane.showMessageDialog(roomFrm, "Chuẩn bị chiến đấu!");
                            roomFrm.updateTbRoom(getRooms());
                        }
                        //Nếu thông điệp xác nhận cho nhập phòng là từ chối
                        else
                            JOptionPane.showMessageDialog(roomFrm, "Yêu cầu bị từ chối!");
                        break;
                    //Xác nhận đấu 1 vs 1
                    case "REPSOLO":
                        msg = data[1].trim();
